...
func (c *DPFMAPICaller) Header(
	...
	where := fmt.Sprintf("WHERE ProductionOrderConfirmation = %d ", input.Header.ProductionOrderConfirmation)
	rows, err := c.db.Query(
		`SELECT *
...
func (c *DPFMAPICaller) HeadersByOwnerProductionPlantBusinessPartner(
	...
	rows, err := c.db.Query(
		`SELECT
		ProductionOrderConfirmationHeader.ProductionOrderConfirmation, 
		ProductionOrderConfirmationHeader.MRPArea, 
		BusinessPartnerGeneralOwnerProductionPlant.BusinessPartnerName as OwnerProductionPlantBusinessPartnerName
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_header_data as ProductionOrderConfirmationHeader
		...
		ON ProductionOrderConfirmationHeader.OwnerProductionPlantBusinessPartner = BusinessPartnerGeneralOwnerProductionPlant.BusinessPartner
		WHERE (ProductionOrderConfirmationHeader.OwnerProductionPlantBusinessPartner) = (?) 
		ORDER BY ProductionOrderConfirmationHeader.IsMarkedForDeletion DESC, ProductionOrderConfirmationHeader.IsLocked DESC, ProductionOrderConfirmationHeader.ProductionVersion DESC;`, ownerProductionPlantBusinessPartner,
	)
	if err != nil {
		*errs = append(*errs, err)
		return nil
...
func (c *DPFMAPICaller) Item(
	...
	for _, v := range input.Header.Item {
		itemIDs = fmt.Sprintf("%s, %d", itemIDs, v.ProductionOrderConfirmationItem)
	}

	if len(itemIDs) != 0 {
		where = fmt.Sprintf("%s\nAND ProductionOrderConfirmationItem IN ( %s ) ", where, itemIDs[1:])
	}
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_data
		`+where+` ORDER BY ProductionOrderConfirmation DESC;`, args...,
	)
	if err != nil {
		*errs = append(*errs, err)
		return nil
...
func (c *DPFMAPICaller) Items(
	...
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_data
		` + where + ` ORDER BY ProductionOrderConfirmation DESC;`)
	if err != nil {
		*errs = append(*errs, err)
		return nil
	}
...
func (c *DPFMAPICaller) ItemComponent(
	...
	for _, v := range item {
		...
		for _, w := range itemOperation {
			...
			for _, x := range itemOperationComponent {
				args = append(args, productionOrderConfirmation, v.ProductionOrderConfirmationItem, x.Operations, x.OperationsItem, x.BillOfMaterial, x.BillOfMaterialItem)
			}
		}
		cnt++
	}
	...
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_component_data
		WHERE (ProductionOrderConfirmation, ProductionOrderConfirmationItem, Operations, OperationsItem, BillOfMaterial, BillOfMaterialItem) IN ( `+repeat+` ) 
		ORDER BY IsMarkedForDeletion ASC, ProductionOrderConfirmation DESC, ProductionOrderConfirmationItem ASC;`, args...,
	)
	if err != nil {
		*errs = append(*errs, err)
		return nil
...
func (c *DPFMAPICaller) ItemComponents(
	...
	where := fmt.Sprintf("WHERE ProductionOrderConfirmation = %d", input.Header.ProductionOrderConfirmation)
	if item != nil {
		if &item.ProductionOrderConfirmationItem != nil {
			where = fmt.Sprintf("%s\nAND ProductionOrderConfirmationItem = %d", where, item.ProductionOrderConfirmationItem)
		}
	}
	...
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_component_data
		` + where + ` ORDER BY IsMarkedForDeletion ASC, ProductionOrderConfirmation DESC, ProductionOrderConfirmationItem ASC;`)
	if err != nil {
		*errs = append(*errs, err)
		return nil
	}
...
func (c *DPFMAPICaller) ItemOperations(
	...
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_operations_data
		` + where + ` ORDER BY ProductionOrderConfirmation DESC, OperationsItem ASC;`)
	if err != nil {
		*errs = append(*errs, err)
		return nil
	}
...
func (c *DPFMAPICaller) ItemComponentDeliveryScheduleLine(
	...
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_component_stock_conf_data
		WHERE (ProductionOrderConfirmation, ProductionOrderConfirmationItem, Operations, OperationsItem, BillOfMaterial, BillOfMaterialItem) IN ( `+repeat+` ) 
		ORDER BY IsMarkedForDeletion ASC, ProductionOrderConfirmation DESC, ProductionOrderConfirmationItem ASC;`, args...,
	)
	if err != nil {
...
func (c *DPFMAPICaller) ItemComponentCosting(
	...
	for _, v := range item {
		...
		for _, w := range itemOperation {
			itemOperationComponent := w.ItemOperationComponent
			for _, x := range itemOperationComponent {
				args = append(args, productionOrderConfirmation, v.ProductionOrderConfirmationItem, x.Operations, x.OperationsItem)
			}
		}
		cnt++
	}
	...
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_component_costing_data
		WHERE (ProductionOrderConfirmation, ProductionOrderConfirmationItem, Operations, OperationsItem) IN ( `+repeat+` ) 
		ORDER BY IsMarkedForDeletion ASC, ProductionOrderConfirmation DESC, ProductionOrderConfirmationItem ASC;`, args...,
	)
	if err != nil {
		*errs = append(*errs, err)
...
func (c *DPFMAPICaller) ItemOperation(
	...
	for _, v := range item {
		itemOperation := v.ItemOperation
		for _, w := range itemOperation {
			args = append(args, productionOrderConfirmation, v.ProductionOrderConfirmationItem, w.Operations, w.OperationsItem)
		}
		cnt++
	}
	repeat := strings.Repeat("(?,?,?,?),", cnt-1) + "(?,?,?,?)"
	...
	rows, err := c.db.Query(
		...
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_operations_data
		WHERE (ProductionOrderConfirmation, ProductionOrderConfirmationItem, Operations, OperationsItem) IN ( `+repeat+` ) 
		ORDER BY ProductionOrderConfirmation DESC, OperationsItem ASC;`, args...,
	)
	if err != nil {
		*errs = append(*errs, err)
		return nil
...
func (c *DPFMAPICaller) ItemOperationComponent(
	...
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_operation_component_data
		WHERE (ProductionOrderConfirmation, ProductionOrderConfirmationItem, Operations, OperationsItem) IN ( `+repeat+` ) 
		ORDER BY ProductionOrderConfirmation DESC, OperationsItem ASC;`, args...,
	)
	if err != nil {
...
func (c *DPFMAPICaller) ItemOperationCosting(
	...
	rows, err := c.db.Query(
		`SELECT *
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_confirmation_item_operation_costing_data
		WHERE (ProductionOrderConfirmation, ProductionOrderConfirmationItem, Operations, OperationsItem) IN ( `+repeat+` ) 
		ORDER BY ProductionOrderConfirmation DESC, OperationsItem ASC;`, args...,
	)
	if err != nil {
